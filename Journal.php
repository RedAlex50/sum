<?php

include('dbconnect.php');

if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    try {
        $stmt = $db->prepare("SELECT id, order_date FROM journal");
        $stmt->execute();
        $values = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        print('Error : ' . $e->getMessage());
        exit();
    }
    
    include('assets/Journal.php');
} else {
    $errors = array();
    $messages = array();

    if (!empty($_POST['addnewdate'])) {
        if (empty($_POST['order_date'])) {
            $errors['order_date'] = 'Заполните поле "Дата заказа"';
        }
        if (empty($_POST['dishes'])) {
            $errors['dishes'] = 'Заполните поле "Список блюд"';
        }
        if (empty($errors)) {
            $order_date = $_POST['order_date'];
            $stmt = $db->prepare("INSERT INTO journal (order_date) VALUES (?)");
            $stmt->execute([$order_date]);

            $journal_id = $db->lastInsertId();
            $dishes = $_POST['dishes'];
            $stmt = $db->prepare("INSERT INTO journal_dishes (journal_id, dish_id) VALUES (?, ?)");
            foreach ($dishes as $dish) {
              $stmt->execute([$journal_id, $dish]);
            }
            $messages['added'] = 'Заказ с <b>id = '.$journal_id.'</b> успешно добавлен';
        }
    }

    foreach ($_POST as $key => $value) {
        if (preg_match('/^clear(\d+)_x$/', $key, $matches)) {
            $id = $matches[1];
            $stmt = $db->prepare("DELETE FROM journal_dishes WHERE journal_id = ?");
            $stmt->execute([$id]);
            $stmt = $db->prepare("DELETE FROM journal WHERE id = ?");
            $stmt->execute([$id]);
            $messages['deleted'] = 'Заказ с <b>id = '.$id.'</b> успешно удалён';
        }

        if (preg_match('/^edit(\d+)_x$/', $key, $matches)) {
            $id = $matches[1];
            setcookie('edit', $id, time() + 24 * 60 * 60);
        }

        if (preg_match('/^save(\d+)_x$/', $key, $matches)) {
            setcookie('edit', '', time() + 24 * 60 * 60);
            $id = $matches[1];
            $stmt = $db->prepare("SELECT order_date FROM journal WHERE id = ?");
            $stmt->execute([$id]);
            $old_dates = $stmt->fetchAll(PDO::FETCH_ASSOC);

            $dates['order_date'] = $_POST['order_date' . $id];

            if (array_diff_assoc($dates, $old_dates[0])) {
                $stmt = $db->prepare("UPDATE journal SET order_date = ? WHERE id = ?");
                $stmt->execute([$dates['order_date'], $id]);
            }
        }
    }
    if (!empty($messages)) {
        setcookie('messages', serialize($messages), time() + 24 * 60 * 60);
    }
    if (!empty($errors)) {
        setcookie('errors', serialize($errors), time() + 24 * 60 * 60);
    }
    header('Location: Journal.php');
}